...
1. **Pick the model + taskType.** Get the model from `runware-models` (live lookup). The `taskType` usually follows the modality: `imageInference` for images, `videoInference` for video, `audioInference` for audio, `3dInference` for 3D, `textInference` for LLMs, `modelUpload` for custom models. But it is a fixed constant declared in each model's own schema, not a modality-level default you can assume - several image models take their own type instead: upscalers use `upscale`, vectorizers use `vectorize`, background removers use `removeBackground`. Edit, inpaint, fill and try-on models stay `imageInference` with a reference image added. Check the model's schema when it is not a plain text-to-X call. A just-launched model not yet in the SDK registry throws "Unknown model" - pass `taskType` explicitly to bypass that.
