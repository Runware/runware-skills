...
Current SDKs: TypeScript is `npm install @runware/sdk` (`import { createClient } from '@runware/sdk'`, `const client = createClient({ apiKey })`) and Python is `pip install runware-sdk` (requires 3.11+, `from runware import Runware`, `async with Runware(api_key=...) as client:`). Both expose a single generic `client.run({ taskType, model, ... })` call that handles every task type — the client resolves the operation from the payload (`taskType` and/or the model's AIR id) rather than exposing one method per `taskType`. Both handle connection/polling bookkeeping for you.

Older packages `@runware/sdk-js` (TypeScript) and `runware` (Python, plain PyPI name, no `-sdk` suffix) still exist and still work, but are the legacy, pre-`run()` SDKs — `@runware/sdk-js` is image-inference-only and no longer receiving new features, and `runware`'s own PyPI listing now points integrators at the new package. Don't recommend either for a new integration; only reach for them if a developer is already on one and asks about it specifically. A new integration should generally reach for the current SDK over raw HTTP unless there's a specific reason not to (no runtime for it, need for a language without an official SDK, etc.).
